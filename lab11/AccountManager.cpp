...
void AccountManager::changeUserName(const string& oldUserName,
	...
	if (myAccounts.find(oldUserName) == myAccounts.end()) {
		throw Exception("changeUserName()",
				"That account is not stored in the manager.");
	} else {
		try {
			createNewAccount(newUserName, myAccounts.find(oldUserName)->second);
		} catch (Exception& e) {
			throw e;
		}
		try {
			myAccounts.erase(myAccounts.find(oldUserName));
		} catch (Exception& e) {
			throw e;
		}
	}
...
void AccountManager::deleteAccount(const string& userName) {
	if (myAccounts.find(userName) == myAccounts.end()) {
		...
		myAccounts.erase(myAccounts.find(userName));
	}
}
