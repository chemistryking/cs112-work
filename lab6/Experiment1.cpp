...
void Experiment1::run() {
	cout << "\n1a. Timing vector append operations using the average over "
...
void Experiment1::timeAppendToListOfSize(unsigned n) {
	cout << " -- time to append to a full list of size " << setw(8) << n
			<< ":\t" << flush;
	myTimer.reset();
	for (int i = 0; i < REPS; i++) {               // repeatedly...
		list<unsigned> l(n);                      // construct new size-n list
...
void Experiment1::timeRepeatedAppendUntilListIsSize(unsigned n) {
	...
	cout << fixed << showpoint << setprecision(9)  // compute average
			<< myTimer.getTotalTime() / n << " secs" << endl;
}
